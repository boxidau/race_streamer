...
class Hmac(object):
    ...
    def get_timestamp(self, request):
        try:
            ts = int(six.b(request.headers["HMACTimestamp"]))
            if (time.time() - 30) > ts:
                raise HmacException("Potential replay attack or misconfigured client")
            return ts
        except KeyError:
            raise SecretKeyIsNotSet()
    ...
    def init_app(self, app):
        ...
        self.hmac_error_code = app.config.get(
            "HMAC_ERROR_CODE", six.moves.http_client.FORBIDDEN
        )
    ...
    def validate_signature(self, request, only=None):
        ...
        if self.hmac_key is not None:
            token = self.make_hmac(signed_data)
        ...
        if self.hmac_keys is not None:
            try:
                ...
                if only is not None:
                    if client in only:
                        token = self.make_hmac_for(client, signed_data)
                        hmac_server_tokens.append(token)
                else:
                    token = self.make_hmac_for(client, signed_data)
                    hmac_server_tokens.append(token)
